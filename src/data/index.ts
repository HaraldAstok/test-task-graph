...
export async function loadAirportData(): Promise<Airport[]> {
	...
	const rows = await parseCSV(
		resolvePath(__dirname, './airports.dat'),
		columns
	);
...
export async function createGraph(routes: Route[]): Promise<NodeMap> {
	const graph: NodeMap = {};
	...
	for (let route of routes) {
		...
		const startValues: NodeWeight = { node: destinationNode, weight: distance };
		const destinationValues: NodeWeight = { node: startNode, weight: distance };

		if (!(startNode in graph)) {
			graph[startNode] = [startValues];
		} else if (
			!graph[startNode].some(
				(elem) =>
					elem.node === startValues.node && elem.weight === startValues.weight
			)
		) {
		...
		if (!(destinationNode in graph)) {
			...
			!graph[destinationNode].some(
				(elem) =>
					elem.node === destinationValues.node &&
